...
def U3_f(rho_W, rho_A, rho_g, T):
  gt = (grad(rho_g) * T + rho_g * grad(T)) * R / W_g
  return - B(rho_W, rho_A) / mu * ( gt + grad(rho_g)*g)
...
# boundary conditions :
T_inf      = Constant(600.0)            # ambient temperatur
...
class BoundarySource(Expression):
  ...
  def eval_cell(self, values, x, ufc_cell):
    ...
    dTdn_x    = kdTdn(T) / k(rho_W, rho_C, rho_A, T, V)[0]
    dTdn_y    = kdTdn(T) / k(rho_W, rho_C, rho_A, T, V)[1]
    c_x       = B(rho_W, rho_A)[0] / mu * rho_g_inf * R / W_g
    c_y       = B(rho_W, rho_A)[1] / mu * rho_g_inf * R / W_g 
    values[0] = c_x * dTdn_x * n[0]
    values[1] = c_y * dTdn_y * n[1]
  def value_shape(self):
    return (2,)

U3_dot_n_T  = BoundarySource(mesh, degree = 2)

G_U3_dot_n  = Constant((0.0, 0.0))
            
bc_U        = DirichletBC(W.sub(0), G_U3_dot_n, ff, 4)
bc_U_T      = DirichletBC(W.sub(0), U3_dot_n_T, entire_boundary)
...
u_p_rho_g     = - B(rho_W, rho_A)[0] / mu * rho_g * T * R / W_g
v_p_rho_g     = - B(rho_W, rho_A)[1] / mu * rho_g * T * R / W_g 

# temperature gradient component of velocity :
u_T           = - B(rho_W, rho_A)[0] / mu * rho_g * T.dx(0) * R / W_g
v_T           = - B(rho_W, rho_A)[1] / mu * rho_g * T.dx(1) * R / W_g 

# boundary gas velocity :
u_p_inf       = - B(rho_W, rho_A)[0] / mu * rho_g_inf * T_inf * R / W_g
v_p_inf       = - B(rho_W, rho_A)[1] / mu * rho_g_inf * T_inf * R / W_g
...
delta_U3      = + dot(U3, phi) * dx \
                + u_p_rho_g * phi[0].dx(0) * dx \
                + v_p_rho_g * phi[1].dx(1) * dx \
